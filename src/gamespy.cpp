...
std::string GameSpy::LoginResponse(const std::string& password, const std::string& uniquenick,
		const std::string& client_challenge, const std::string& server_challenge)
{
	const std::string space = "                                                ";
	
	return Util::MD5hash(password + space + uniquenick + client_challenge + server_challenge + password);
}
